...
namespace toadll::mappings
{
	...
	std::unordered_map<mapping, const char*> fieldnames;
	std::unordered_map<mapping, const char*> fieldsigs;
	...
	void init_map(JNIEnv* env, jclass mcclass, minecraft_client client)
	{
		...
		{
			fieldnames.insert({ mapping::theMcField, "S" });
			fieldsigs.insert({ mapping::theMcField, "Lave;" });
